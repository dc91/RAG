...
# from openai import OpenAI # Removed: No longer using OpenAI for embeddings
...
# Added: Import SentenceTransformer
...
# -------------------Config----------------------#
# -----------------------------------------------#
# OPENAI_KEY = os.getenv("OPENAI_API_KEY") # No longer strictly needed for embeddings, but might be for other OpenAI features
...
# EMBEDDING_MODEL_NAME = "text-embedding-3-small" # OpenAI specific model
TOKEN_ENCODER = tiktoken.encoding_for_model(
    "gpt-3.5-turbo"
)  # tiktoken can still be used for token counting, typically matches GPT models
MAX_TOKENS = 512
...
# This model is a good general-purpose embedding model.
# You can choose other models from https://www.sbert.net/docs/pretrained_models.html
sentence_transformer_model = SentenceTransformer("all-MiniLM-L6-v2")


# Define a custom embedding function for ChromaDB using Sentence Transformers
...
collection = chroma_client.get_or_create_collection(
    ...
    embedding_function=sentence_transformer_ef,  # Use the new embedding function
)

# client = OpenAI(api_key=OPENAI_KEY) # Removed: Not using OpenAI for embeddings anymore
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".pdf"):
            ...
            for chunk in chunks:
                ...
                # The embedding is generated by the model itself, not client.embeddings.create
                embedding = sentence_transformer_model.encode(chunk_text).tolist()
                ...
                collection.upsert(
                    ...
                    embeddings=[embedding],  # Use the generated embedding
    ...
    for filename in os.listdir(toml_dir):
        if filename.endswith(".toml"):
            ...
            for question in toml_f["questions"]:
                # Generate embedding for question using Sentence Transformer
                question["question_embedding"] = sentence_transformer_model.encode(
                    question["question"]
                ).tolist()
...
def check_shrinking_matches(text_list, chunk, shrink_from_start=False):
    ...
    for i in range(text_len):
        ...
        if substring in chunk:
            ...
            print(
                "Text from chunk: ",
                chunk[max(idx - 50, 0) : min(len(chunk), idx + 200)],
            )
            return True
    return False
...
def match_strings(chunk_text, answer):
    ...
    if check_shrinking_matches(
        answer_chars, chunk_text.replace("\n", " "), shrink_from_start=False
    ):
...
def query_documents(
    question_text, n_results=3
):  # Renamed 'question' to 'question_text' for clarity
    # When query_texts is used, ChromaDB will use the collection's configured embedding function.
...
# --------------------------------------------------------------------#
# Uncomment this to process your PDFs and generate embeddings
process_pdfs_and_insert(PDF_DIRECTORY)

# --------------------------------------------------------------#
# ----------------Write a question, Run a query-----------------#
# --------------------------------------------------------------#
# Example of querying with a text string directly (ChromaDB will embed it using the set embedding function)
...
# --------------------------------------------------------------#
# Uncomment this if you want to embed your questions in the TOML files
# add_embeddings_to_toml(TOML_DIRECTORY)

# --------------------------------------------------------------#
# -------Get the data from toml files, with embedding-----------#
# --------------------------------------------------------------#
# Uncomment this to load questions from your TOML files
question_dict = get_embedded_questions(TOML_DIRECTORY)

# --------------------------------------------------------------#
# ------------------Run a query from toml files-----------------#
# --------------------------------------------------------------#
# Uncomment and provide a valid question ID from your TOML files
print(question_dict["PMCSKOLVERKET001"])
q_doc(question_dict["PMCSKOLVERKET001"])
