    ...
    for question in tqdm(question.values(), desc="Processing questions"):
        ...
        if RERANK:
            if RERANK_MODEL == "COHERE":
                func = rerank_cohere
            elif RERANK_MODEL == "JINA_API":
                func = rerank_jina_api
            # Don't uncomment unless you want to download and use it, this will take time since it needs to download and eval the model
            # elif RERANK_MODEL == "JINA_LOCAL":
            #     func = rerank_jina_local
            else:
                func = rerank_cohere
                print(
                    "Warning, Used cohere rerank model as default, could not recognize RERANK_MODEL, check config file"
                )
        ...
        for idx, document in enumerate(results_to_use):
            ...
            # we need the find the file wich matches the guessed file, if there is a match.
            filename_match = guessed_file in correct_files
            if filename_match:
            ...
            result_id = f"{question['id']}R{idx + 1}"
            ...
            if USE_LLM_ANSWERS:
                results_after_loop = idx + 1
                answer = generate_response_from_context(
                    question["question"], results["documents"][0][:2], SYS_PROMPT_FOR_OUTPUT
                )
                for row in all_rows[-results_after_loop:]:
                    row.append(answer)
